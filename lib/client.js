...
class FormletClient {
  ...
  /*
   * Performs a RAW FIND, giving the entire query object to Formlet, using the Sequelize object
   * */
  findRaw(payload, filter) {
    if (typeof payload !== 'object' || !payload) payload = {};
    _setFormlet(this, payload);
    return this.request('entity.raw.find', this._payload, arguments);
  }
  ...
  /**
   * Count the number of items, with the similar query system of find()
   * */
  count(payload, filter) {
    if (typeof payload !== 'object' || !payload) payload = {};
    _setFormlet(this, payload);
    return this.request('entity.count', this._payload, arguments);
  }
